...
class rdcProtocol(serverProtocol.RDCServerProtocol):
    ...
    def __init__(self):
        ...
        self._maxWidth = app.desktop().size().width()
        self._maxHeight = app.desktop().size().height()
    ...
    def _makeFramebuffer(self, width, height, quality):
        pix = QScreen.grabWindow(
        ...
        pix.save(self._buffer, 'JPG', quality)
...
class RDCServerGUI(QDialog):
    ...
    def setupUI(self):
        ...
        self.setStyleSheet(open(os.path.abspath(
            os.path.dirname(__file__)) + '/styleSheet.qss', 'r').read())
